  ...
  const Likes = () => {
    if (likes.length > 0) {
      return likes.find(oneLike => oneLike === userId)
        ...
        <> <ThumbUpOffAltIcon fontSize='medium' /> <span style={myStyle.spanStyle}> {likes.length} </span> </>
  ...
  return (
    <>
      <Card style={myStyle.card} >
        ...
        <CardContent>
          <Typography variant="body2" color="textSecondary" component={"span"} >{post.message.slice(0, 300)}  {post.message.length > 300 && <><Typography onClick={handleMessageClick} sx={{ color: blue[500], cursor: "pointer" }} > Read More..... </Typography></>}  </Typography>
